...
const swap = ([a, b]: [Dice, Dice]): [Dice, Dice] => [b, a]
const shuffle = (a: DiceBag) => {
  a.reverse().forEach((_, i) => {
    const j = Math.floor(Math.random() * (i + 1))
    ;[a[i], a[j]] = swap([a[i] as Dice, a[j] as Dice])
  })
  return a
}
...
export const caclulatePointsEarned = (n: number): number => {
  if (n < 3) return 0
  if (n < 5) return 1
  if (n === 5) return 2
  if (n === 6) return 3
...
export const makeGetAdjacent =
  ...
  (i: number): AdjacencyTuple => {
    const min = 0
    const max = c * r
    if (i == null || i < min || i > max) return [-1, -1, -1, -1, -1, -1, -1, -1]
    return [
      i % c === 0 || i < r ? -1 : i - c - 1,
      i < r ? -1 : i - c,
      (i + 1) % c === 0 || i < r ? -1 : i - c + 1,
